...
namespace FYP.Library.Encryption
{
    class EncryptionRuntime
    {
        private static string PasswordSalt = "default_felix_method"; //salt key, use longer salt to having strong protection
        private static int bufferLen = 4096, // 4K Length
            bytesRead = 0; // use for file encrypt or decrypt don't modified
        ...
        // define salt new key
        public EncryptionRuntime(string Salt = null)
        {
            try
            {
        ...
        public static byte[] Encrypt(byte[] RawData, byte[] Key, byte[] IV)
        {
            try
            {
                ...
                using (MemoryStream ms = new MemoryStream()) // Create a MemoryStream to accept the encrypted bytes 
                {
        ...
        public string Encrypt(string RawData) // fix here add default password
        {
            try
            {
                ...
                using (PasswordDeriveBytes pdb = new PasswordDeriveBytes(PasswordSalt, new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 }))
                {
        ...
        public byte[] Encrypt(byte[] RawData) // fix here add default password
        {
            try
            {
                // If Cipher data is empty throw it
                if (RawData.Length == 0) { throw new Exception("Cipher data cannot be null"); }
        ...
        public void Encrypt(string fileIn, string fileOut) // fix here add default password
        {
            try
            {
                ...
                // First we are going to derive a Key and an IV from the Password and create an algorithm 
                using (PasswordDeriveBytes pdb = new PasswordDeriveBytes(PasswordSalt, new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 }))
                {
                    // Then we are going to open the file streams in (read), out (write)
        ...
        public string Decrypt(string cipherData) // fix here add default password
        {
            ...
            {
                if (cipherData.Length == 0) { throw new Exception("Cipher data cannot be null"); } // If Cipher data is empty throw it
                byte[] cipherBytes = Convert.FromBase64String(cipherData), // First we need to turn the input string into a byte array. We presume that Base64 encoding was used 
                    decryptedData; // Variables
