/* Function List
 * ==============
 * Felix.Library.Common.Encryption
 *      CommonEncryption(string Salt)
 *      Encrypt(byte[] RawData, byte[] Key, byte[] IV)
 *      Encrypt(string RawData)
 *      Encrypt(byte[] RawData)
 *      Encrypt(string fileIn, string fileOut)
 *      Decrypt(byte[] cipherData, byte[] Key, byte[] IV)
 *      Decrypt(string cipherData)
 *      Decrypt(byte[] cipherData)
 *      Decrypt(string fileIn, string fileOut)
 */
using System;
using System.IO;
using System.Text;
...
namespace Felix.Library.Common.Encryption
{
    public class CommonEncryption
    {
        ...
        public static byte[] Encrypt(byte[] RawData, byte[] Key, byte[] IV)
        {
            ...
            try
            {
                ...
                cs.Write(RawData, 0, RawData.Length); // Write the data and make it do the encryption 
            ...
            catch { throw new Exception("Encryption Error"); }
        }
        ...
        public static byte[] Decrypt(byte[] cipherData, byte[] Key, byte[] IV)
        {
            ...
            catch { throw new Exception("Decryption Error"); }
        }
        ...
        public void Decrypt(string fileIn, string fileOut) // fix here add default password
        {
            ...
            try
            {
                FileStream fsIn = new FileStream(fileIn, FileMode.Open, FileAccess.Read), fsOut = new FileStream(fileOut, FileMode.OpenOrCreate, FileAccess.Write); // First we are going to open the file streams, in (read), out (write)
                PasswordDeriveBytes pdb = new PasswordDeriveBytes(PasswordSalt, new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 }); // Then we are going to derive a Key and an IV from the Password and create an algorithm 
                alg.Key = pdb.GetBytes(32);
                alg.IV = pdb.GetBytes(16);
                ...
                cs.Close(); fsIn.Close(); // close everything, this will also close the unrelying fsOut stream 
            }
            catch { throw new Exception("Decryption Error"); }
        }
    }
